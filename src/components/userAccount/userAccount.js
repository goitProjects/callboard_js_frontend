...
function handleCLickOpenModalAcc(e){
  if(e.target.classList == "logged-user")
  {
    ...
    services.getFavorites({headers: {Authorization:tokenUserAcc}}).then(res => {
      const Datafav=res.data.user.favorites;
      document.querySelector(".userAccount__favorite-list").insertAdjacentHTML("beforeend", favHbs(Datafav))
     });
